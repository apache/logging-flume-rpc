...
public class NettyAvroRpcClient extends SSLContextAwareAbstractRpcClient {
    ...
    private void connect(long timeout, TimeUnit tu) throws FlumeException {
        callTimeoutPool =
                Executors.newCachedThreadPool(new TransceiverThreadFactory("Flume Avro RPC Client Call Invoker"));

        try {
            transceiver = new NettyTransceiver(this.address, Math.toIntExact(tu.toMillis(timeout)), (ch) -> {
        ...
        try {
            ...
            if (t instanceof TimeoutException) {
                throw new EventDeliveryException(
                        this + ": Failed to send event. " + "RPC request timed out after " + requestTimeout + "ms", t);
            }
    ...
    private SSLEngine createSSLEngine() {
        ...
        try {
            if (enableSsl) {
                if (trustAllCerts) {
                    logger.warn("No truststore configured, setting TrustManager to accept all server certificates");
