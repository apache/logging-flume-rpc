...
public class AvroCLIClient {
    ...
    private boolean parseCommandLine(String[] args) throws ParseException {
        ...
        options.addOption("P", "rpcProps", true, "RPC client properties file with " + "server connection params")
                ...
                .addOption(
                        "R", "headerFile", true, ("file containing headers as " + "key/value pairs on each new line"))
            ...
            new HelpFormatter()
                    .printHelp(
                            ...
                            "The --dirname option assumes that a spooling directory exists "
                                    + "where immutable log files are dropped.",
                            true);
        ...
        if (commandLine.hasOption("rpcProps")) {
            ...
            Preconditions.checkNotNull(
                    rpcClientPropsFile, "RPC client properties " + "file must be specified after --rpcProps argument.");
            Preconditions.checkArgument(
                    new File(rpcClientPropsFile).exists(),
                    "RPC client properties file %s does not exist!",
                    rpcClientPropsFile);
        }
    ...
    private void run() throws IOException, FlumeException, EventDeliveryException {
        ...
        if (rpcClientPropsFile != null) {
            ...
            rpcClient = RpcClientFactory.getDefaultInstance(hostname, port, BATCH_SIZE);
        }
