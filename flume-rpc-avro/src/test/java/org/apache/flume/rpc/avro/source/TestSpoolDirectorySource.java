...
public class TestSpoolDirectorySource {
    ...
    public void testRecursion_SetToTrue() throws IOException, InterruptedException {
        ...
        Context context = new Context();
        context.put(
                SpoolDirectorySourceConfigurationConstants.RECURSIVE_DIRECTORY_SEARCH,
        ...
        Configurables.configure(source, context);
        source.start();
        Assert.assertTrue("Recursion setting in source is correct", source.getRecursiveDirectorySearch());
        ...
        Assert.assertNotNull("Event headers must not be null", e.getHeaders());
        Assert.assertTrue(
                "File header value did not end with expected filename",
        ...
        Assert.assertEquals(
                "Event body is correct", Arrays.toString(origBody.getBytes()), Arrays.toString(baos.toByteArray()));
        txn.commit();
        txn.close();
    }
    ...
    public void testRecursion_SetToFalse() throws IOException, InterruptedException {
        ...
        context.put(SpoolDirectorySourceConfigurationConstants.RECURSIVE_DIRECTORY_SEARCH, "false");
        context.put(SpoolDirectorySourceConfigurationConstants.SPOOL_DIRECTORY, tmpDir.getAbsolutePath());
        context.put(SpoolDirectorySourceConfigurationConstants.FILENAME_HEADER, "true");
        context.put(SpoolDirectorySourceConfigurationConstants.FILENAME_HEADER_KEY, "fileHeaderKeyTest");
        ...
        Assert.assertFalse(
                "Recursion setting in source is not set to false (this" + "test does not want recursion enabled)",
    ...
    public void testSourceDoesNotDieOnFullChannel() throws Exception {
        ...
        Assert.assertTrue("Expected to hit ChannelFullException, but did not!", source.didHitChannelFullException());
    ...
    public void testEndWithZeroByteFiles() throws IOException, InterruptedException {
        ...
        Assert.assertEquals(
                "Four messages were read", 4, source.getSourceCounter().getEventAcceptedCount());
        source.stop();
    }
    ...
    public void testErrorCounters() throws Exception {
        ...
        Mockito.doThrow(new ChannelException("dummy"))
                ...
                .when(cp)
                .processEventBatch(ArgumentMatchers.anyList());
