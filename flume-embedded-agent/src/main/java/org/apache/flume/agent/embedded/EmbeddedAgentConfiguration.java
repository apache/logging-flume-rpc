...
public class EmbeddedAgentConfiguration {
    ...
    private static final String[] ALLOWED_SOURCES = {
        SOURCE_TYPE_EMBEDDED_ALIAS, SOURCE_TYPE_EMBEDDED,
    };
        ...
        for (String sink : sinkNames.split("\\s+")) {
            if (DISALLOWED_SINK_NAMES.contains(sink.toLowerCase(Locale.ENGLISH))) {
                throw new FlumeException(
                        "Sink name " + sink + " is one of the" + " disallowed sink names: " + DISALLOWED_SINK_NAMES);
            }
    ...
    static Map<String, String> configure(String name, Map<String, String> properties) throws FlumeException {
        ...
        result.put(join(name, BasicConfigurationConstants.CONFIG_SINKGROUPS, sinkGroupName, SINKS), sinkNames);
        ...
        for (String key : userProvidedKeys) {
            ...
            if (key.startsWith(SOURCE_PREFIX)) {
                ...
                result.put(join(name, BasicConfigurationConstants.CONFIG_SINKGROUPS, sinkGroupName, key), value);
            } else {
    ...
    private static void checkAllowed(String[] allowedTypes, String type) {
        ...
        if (!isAllowed) {
            throw new FlumeException(
                    "Component type of " + type + " is not in " + "allowed types of " + Arrays.toString(allowedTypes));
        }
    }
